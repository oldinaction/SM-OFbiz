    ...
    <table width="500" border="0">
      <tr>
        <td><a href="http://www.lightstreamer.com" target="_blank">
          LIGHTSTREAMER</a></td>
        <td class="demoTitle">Remote Adapter</td>
      </tr>
    </table>
...
//////////////// 发送消息Message Submission Form Handling
  var client = null;
  require(["/tomahawk/lightstreamer/js/lsClientRemote.js"],function(lsClient) {
  ...
  function submitForm() {
    ...
    if (window.client && textField) {
      ...
      if (!text) {
        ...
        var mex = "REMOTE|admin|" + text; // 消息主体
        client.sendMessage(mex);
      }
    }
  }

//////////////// 订阅消息Message Grid and Subscription Management

  require(["/tomahawk/lightstreamer/js/lsClientRemote.js","DynaGrid","Subscription"],function(lsClient,DynaGrid,Subscription) {
    //  create the DynaGrid
    ...
    // new一个订阅对象，参数二为订阅人id
    var chatSubscription = new Subscription("DISTINCT","sm_admin",chatGrid.extractFieldList());
    chatSubscription.setDataAdapter("MY_REMOTE"); // adapter.xml中data_provider的name
